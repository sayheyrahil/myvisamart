...
const typeOptions = [
  ...
  { value: "Visamart-US Visa Mock Interview", label: "Visamart-US Visa Mock Interview" },
  { value: "Visamart-Transparency Hub", label: "Visamart-Transparency Hub" },
  { value: "DS-160", label: "DS-160" },
  { value: "Schengen Appointment Checker", label: "Schengen Appointment Checker" },
  { value: "Schengen Cover Letter", label: "Schengen Cover Letter" },
  { value: "Schengen Invitation Letter", label: "Schengen Invitation Letter" },
  { value: "UAE Visa Status Online", label: "UAE Visa Status Online" },
  { value: "Vietnam Visa Status Checker", label: "Vietnam Visa Status Checker" },
  { value: "Visa Eligibility Quiz", label: "Visa Eligibility Quiz" },
  { value: "Visa Glossary", label: "Visa Glossary" },
  { value: "Visa Glossary Adjustment of Status", label: "Visa Glossary Adjustment of Status" },
  { value: "Visa Photo Maker", label: "Visa Photo Maker" },
];
...
export default function Page({ params: paramsPromise }: { params: any }) {
  ...
  const fetchData = useCallback(
    (id: number) => {
      const idpass = { id: id };
      axiosInstance.post(ENDPOINTS.faq_edit, idpass)
        .then((response: any) => {
  ...
  const onSubmit = async (data: faqForm) => {
    ...
    submitData.type = Array.isArray(submitData.type) ? submitData.type.join(",") : submitData.type;
    await axiosInstance.post(ENDPOINTS.faq_store, submitData)
  ...
  function validate(): string {
    ...
    if (form.question.length < 5) return "Question must be at least 5 characters."
