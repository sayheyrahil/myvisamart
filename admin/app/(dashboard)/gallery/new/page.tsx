...
export default function NewGalleryPage() {
  ...
  const [imagesUploading, setImagesUploading] = React.useState<boolean>(false)
  ...
  async function getEditData() {
    if (id) {
      ...
      try {
        let idpass = `?id=${id}`;
        const response = await axiosInstance.get(`${ENDPOINTS.gallery_edit}${idpass}`)
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      // Create new or update gallery
      await axiosInstance.post(ENDPOINTS.gallery_store, values)
        .then((response) => {
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="sm:col-span-2">
                      ...
                      <FormDescription>
                        This will be displayed as the main title of your gallery item.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="sm:col-span-2">
                      ...
                      <FormControl>
                        <ImageUpload
                          ...
                          onChange={(urlOrUrls, previewOrPreviews) => {
                            // Always treat as array for form value and preview
                            ...
                            form.setValue("images", urlsArray)
                            setImagesPreview(previewsArray)
                          }}
                      ...
                      <FormDescription>
                        Upload one or more images for this gallery item. You can select multiple images at once.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button type="submit" disabled={isLoading}>
              {isLoading ? (isEdit ? "Updating..." : "Creating...") : (isEdit ? "Update Item" : "Create Item")}
            </Button>
          </div>
        </form>
      </Form>
