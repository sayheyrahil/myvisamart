...
import HappyClientModel from "@models/happy-client-model"; // <-- Change to your HappyClient model
import commonFunction from "@helpers/commonFunction";

type HappyClientData = {
    id?: number;
    name: string;
    description: string;
    designation: string;
...
const get = async (req: Request, res: Response) => {
    try {
        ...
        const [items, totalCount] = await Promise.all([
            HappyClientModel.findAll(queryOptions),
            HappyClientModel.count({ where }),
        ]);
...
const destroy = async (req: Request, res: Response) => {
    try {
        ...
        const removeImgModel = await HappyClientModel.findByPk(id);

        const numDeleted = await HappyClientModel.destroy({
            // @ts-ignore
            where: {
                id: id,
            },
...
const getData = async (id: any) => {
    try {
        const itemData = await HappyClientModel.findByPk(id, {
            attributes: project,
        });
...
const store = async (req: Request, res: Response) => {
    ...
    try {
        const bodyData: HappyClientData = {
            name: req.body.name,
        ...
        const [itemData, created] = await HappyClientModel.upsert(bodyData, {
