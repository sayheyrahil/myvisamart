...
const get = async (req: Request, res: Response) => {
    try {
        ...
        const [items, totalCount] = await Promise.all([
            FaqModel.findAll(queryOptions),
            FaqModel.count({ where }),
        ]);
...
const destroy = async (req: Request, res: Response) => {
    try {
        ...
        const numDeleted = await FaqModel.destroy({
            where: {
                id: id,
            },
        });
...
const getData = async (id: any) => {
    try {
        const itemData = await FaqModel.findByPk(id, {
            attributes: project,
        });
...
const store = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [itemData, created] = await FaqModel.upsert(bodyData, {
