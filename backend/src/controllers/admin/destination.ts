...
import DestinationModel from "@models/destination-model"; // <-- Change to your HappyClient model
...
const get = async (req: Request, res: Response) => {
    try {
        ...
        const [items, totalCount] = await Promise.all([
            DestinationModel.findAll(queryOptions),
            DestinationModel.count({ where }),
        ]);
...
const destroy = async (req: Request, res: Response) => {
    try {
        ...
        const removeImgModel = await DestinationModel.findByPk(id);

        const numDeleted = await DestinationModel.destroy({
            // @ts-ignore
            where: {
                id: id,
            },
...
const getData = async (id: any) => {
    try {
        const itemData = await DestinationModel.findByPk(id, {
            attributes: project,
        });
...
const store = async (req: Request, res: Response) => {
    ...
    try {
        const bodyData: DestinationData = {
            name: req.body.name,
        ...
        const [itemData, created] = await DestinationModel.upsert(bodyData, {
