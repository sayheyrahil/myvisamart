...
import OurServiceModel from "@models/our-service-model"; // <-- Change to your OurService model
...
const get = async (req: Request, res: Response) => {
    try {
        ...
        const [items, totalCount] = await Promise.all([
            OurServiceModel.findAll(queryOptions),
            OurServiceModel.count({ where }),
        ]);
...
const destroy = async (req: Request, res: Response) => {
    try {
        ...
        const removeImgModel = await OurServiceModel.findByPk(id);

        const numDeleted = await OurServiceModel.destroy({
            // @ts-ignore
            where: {
                id: id,
            },
...
const getData = async (id: any) => {
    try {
        const itemData = await OurServiceModel.findByPk(id, {
            attributes: project,
        });
...
const store = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [itemData, created] = await OurServiceModel.upsert(bodyData, {
