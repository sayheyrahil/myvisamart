...
import TeamModel from "@models/team-model"; // <-- Change to your Team model
...
const get = async (req: Request, res: Response) => {
    try {
        ...
        const [items, totalCount] = await Promise.all([
            TeamModel.findAll(queryOptions),
...
const destroy = async (req: Request, res: Response) => {
    try {
        ...
        const removeImgModel = await TeamModel.findByPk(id);

        const numDeleted = await TeamModel.destroy({
            // @ts-ignore
            where: {
                id: id,
            },
...
const getData = async (id: any) => {
    try {
        const itemData = await TeamModel.findByPk(id, {
            attributes: project,
        });
...
const store = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [itemData, created] = await TeamModel.upsert(bodyData, {
