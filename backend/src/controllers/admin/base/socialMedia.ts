...
const titleName = "Social Media";
const allFiled = ["id", "platform", "url", "is_active", "createdAt"];
const project: any = {};
...
const get = async (req: Request, res: Response) => {
  try {
    ...
    const [items, totalCount] = await Promise.all([
      SocialMediaModel.findAll(queryOptions),
      SocialMediaModel.count({ where }),
    ]);
...
const destroy = async (req: Request, res: Response) => {
  try {
    ...
    const numDeleted = await SocialMediaModel.destroy({
      // @ts-ignore
      where: {
        id: id,
      },
...
const getData = async (id: any) => {
  try {
    const itemData = await SocialMediaModel.findByPk(id, {
      attributes: project,
    });
...
const store = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [itemData, created] = await SocialMediaModel.upsert(bodyData, {
