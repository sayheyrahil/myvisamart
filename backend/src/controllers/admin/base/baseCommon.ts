...
import { reSizeImage } from "@helpers/sizeImage";
...
const uploadFiles = async (req: any, res: any) => {
    try {
        ...
        if (imagePath && blob) {
            ...
            const uploadedImageData: any = imageName;
...
const uploadMedia = async (req: any, res: any) => {
    try {
        if (!req.file) {
            return response.sendError(res, { message: "No file uploaded" });
        }

        const baseUrl =
            process.env.API_URL +
            `/uploads/${req.query.type}/` +
            req.file.filename;

        const storeURL = `/uploads/${req.query.type}/` + req.file.filename;
        ...
        logger.info("uploadMedia error", err);
...
const uploadVideo = async (req: any, res: any) => {
    try {
        if (!req.file) {
            return response.sendError(res, { message: "No video file uploaded" });
        }
