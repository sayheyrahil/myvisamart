...
const get = async (req: Request, res: Response) => {
  try {
    ...
    const [items, totalCount] = await Promise.all([
      CmsPagesModel.findAll(queryOptions),
      CmsPagesModel.count({ where }),
    ]);
...
const destroy = async (req: Request, res: Response) => {
  try {
    ...
    const numDeleted = await CmsPagesModel.destroy({
      // @ts-ignore
      where: {
        id: id,
      },
...
const getData = async (id: any) => {
  try {
    const itemData = await CmsPagesModel.findByPk(id, {
      attributes: project,
    });
...
const store = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [itemData, created] = await CmsPagesModel.upsert(bodyData, {
