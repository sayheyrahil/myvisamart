...
import TestimonialModel from "@models/testimonials-model"; // <-- Add this import
...
const getActiveTestimonial = async (req: Request, res: Response) => {
  try {
    const { type } = req.body;
    const { Op } = require("sequelize");
    let where: any = { is_active: true };
    ...
    const testimonials: any = await TestimonialModel.findAll({ where });
    const sendResponse: any = {
      data: testimonials,
      message: "get successfully",
    };
    ...
    logger.info("get testimonial error:", err);
    logger.info(err);
    return response.sendError(res, sendResponse);
  }
};
...
export default {
  ...
  getCountryDetail,
  getActiveTestimonial, // <-- Add this export
};
